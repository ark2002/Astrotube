...
const AddToPlayListCard = ({ oneplaylist, setError }) => {
  ...
  return (
    <div
      className="playlistCard__container flex--row"
      onClick={() =>
        addVideoToPlaylistHandler(videoIsThere, _id, playlists.currentVideo)
      }
    >
      {videos.length > 0 ? (
        <img
          src={`http://img.youtube.com/vi/${videos[0]._id}/maxresdefault.jpg`}
          alt="thubmnail"
          className="playlist__img"
        />
      ) : (
        <img
          src="assets\AstroTube__Thumbnail.jpeg"
          alt="thubmnail"
          className="playlist__img"
        />
      )}
      <div className="playlistCard__details flex--column">
        ...
        <p className="playlist__info secondary__font text__small">
          {description}
        </p>
      </div>
    </div>
