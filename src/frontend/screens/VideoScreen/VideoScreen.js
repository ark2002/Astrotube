...
const VideoScreen = () => {
    ...
    return (
        playerVideo && <div className="video-screen__container flex--row">
            <section className="video__main flex--column">
                <VideoPlayer video={playerVideo} />
            </section>
            <aside className="video__recommendations flex--column">
                {videos.filter((video) => video.category === playerVideo.category && video._id !== videoId).map((video) => <RecommendationCard video={video} key={video._id} />)}
            </aside>
        </div>
    );
}
